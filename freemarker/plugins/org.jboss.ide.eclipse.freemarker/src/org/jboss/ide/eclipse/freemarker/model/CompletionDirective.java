...
public class CompletionDirective extends AbstractDirective {
	...
	public CompletionDirective (int offset, int length, ItemSet itemSet, ISourceViewer viewer, IResource resource) throws BadLocationException {
		contents = viewer.getDocument().get(offset, length);
		ITypedRegion region = new TypedRegion(offset, contents.length(), "default"); //$NON-NLS-1$
		this.offset = offset;
		this.length = length;
		...
		if (null != arr && arr.length > 0) {
			...
			try {
				if (s.equals("list")) { //$NON-NLS-1$
					...
					directive.load(new TypedRegion(offset, contents.length(), "list"), viewer, resource); //$NON-NLS-1$
					directive.setItemSet(itemSet);
				}
				else if (s.equals("if")) { //$NON-NLS-1$
					directive = new IfDirective();
					directive.load(new TypedRegion(offset, contents.length(), "if"), viewer, resource); //$NON-NLS-1$
					directive.setItemSet(itemSet);
				}
			}
			catch (Exception e) {
